...
- `ui/main_window.py` 目前不是正式入口顯示的 UI，後續 redesign 不應以它為主要目標。
...
- `core/config_manager.py`：舊版 / 補充設定，讀寫 `resources/config.json`。目前仍用於 startup video 等設定。
...
- `assets/themes/`：輪盤主題素材。
  ...
  - `lava/`
  - `cosmic/`
  - 每個主題可包含 `rim.png`、`rim_active.png`、`card_bg.png`、`frames/frame_000.png` 到 `frame_023.png`。
- `assets/startup/startup.mp4`：啟動影片。
- `assets/emoji_catalog.json`：早期 / 補充 icon catalog。
...
- `resources/config.json`：舊版 / 補充設定，包含 startup video 設定等。
- `resources/styles/dark.qss`、`resources/styles/light.qss`：樣式資源。
- `resources/ui.png`、`resources/uitest.png`：UI 參考圖。
...
4. `Application.run()`：
   ...
   - 建立 / 註冊 global hotkey。
   - 建立 tray icon。
   - 顯示 startup video splash，或直接進入背景。
   - App 留在背景等待 hotkey。
...
| `resources/config.json` | 舊版 / 補充設定，例如 startup video |
| `data/powershell_library.json` | PowerShell Library 腳本 |
...
- Startup video：`assets/startup/startup.mp4`
...
- UI 參考圖：`resources/ui.png`、`resources/uitest.png`
...
Startup Video Splash 會在第一次啟動 SmartAction 時播放 `assets/startup/startup.mp4`，最多 5 秒。播放完或按 Skip 後，App 進入背景 / system tray，不會自動彈出主視窗。若影片不存在或播放失敗，會直接跳過，不影響啟動。Phase 4 後 splash 會透過目前螢幕的 availableGeometry() 自動限制大小並置中，避免在 1080p / 2K 螢幕間跑版。
