(() => {
  ...
  async function runPowerShellScript(event) {
    event.preventDefault(); const script = state.psRunScript; if (!script || state.psRunning) return;
    const values = Object.fromEntries([...$("#ps-parameter-fields").querySelectorAll("input")].map((input) => [input.name, input.value]));
    const submit = $("#ps-confirm-run"); const originalLabel = submit.textContent;
    state.psRunning = true; submit.disabled = true; submit.textContent = "Core 執行中…";
    ...
    finally { state.psRunning = false; submit.disabled = false; submit.textContent = originalLabel; }
  }
  function showPowerShellResult(name, value, failed) { const result = $("#ps-run-result"); result.classList.remove("hidden"); $("#ps-result-title").textContent = `${failed ? "Failed" : "Completed"}: ${name}`; const output = [value.stdout, value.stderr && `stderr:\n${value.stderr}`, value.friendly_error].filter(Boolean).join("\n\n") || (failed ? "The Core execution request failed." : "Script completed without output."); $("#ps-result-output").textContent = output; result.scrollIntoView({ behavior: "smooth", block: "start" }); }
