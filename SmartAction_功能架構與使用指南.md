...
- 正式啟動方式：`Set-Location C:\Users\naugh\smartaction; python -m app.main`。
- SmartAction 目前是 tray + hotkey 型 Windows productivity launcher，不是固定顯示的主視窗工具。
- 正式 UI 入口包含：System Tray、Universal Actions Ring、Settings、PowerShell Library、Client Workspace、Environment Check、Help、Emoji Picker、Hotkey Picker。
- UI redesign 已完成 Phase 1 到 Phase 5：Settings、Ring、supporting dialogs、display stability、tray menu、startup splash 與 release-readiness 自動 QA 都已完成。
- PowerShell Library action type 目前在 Ring 上的行為是「開啟 PowerShell Library 視窗」。尚未支援把某一條 Library script 直接綁到 Ring 並一鍵執行。
- Phase 5 自動 QA 已通過 Python compile、主要 UI offscreen 建構、action registry、PowerShell Library action callback 與主要 JSON 設定讀取；正式 release 前仍需人工實機測試 tray、hotkey、startup splash、F11/Esc、1080p / 2K / DPI scaling。
...
| PowerShell Library | 開啟 PowerShell Library 管理視窗 | 需要從輪盤快速打開腳本庫、查找或手動執行常用 IT 腳本 | 不需 target / script | 打開腳本庫後再選 Ping Target、Create Local User、Join Domain |
...
PowerShell Library Type 目前會開啟 PowerShell Library 視窗。輪盤 action 不需要 target，也不需要選擇 `script_id`。

這代表你可以把「腳本庫入口」放到 Ring 上，點擊後進入完整的 PowerShell Library 管理與執行介面。

目前尚未完成的能力：直接把某一條 Library script 綁到 Ring，並在點擊 Ring action 時直接執行該 script。
...
| 設定項目 | 說明 |
| --- | --- |
| Script | 目前不需要選擇；Ring action 只負責開啟 PowerShell Library 視窗 |
| Display Name | Ring 上顯示的 action 名稱 |
| Icon / Emoji | 顯示圖示 |
| script_id | 目前不使用；保留給未來直接綁定 Library script 的功能 |
...
- Skip 後進入背景，不退出 App。
- 影片不存在時自動跳過。
- Phase 4 後會依目前螢幕可用區域自動 clamp 尺寸並置中，避免 1080p / 2K 螢幕間跑版。
- Splash host、狀態文字與 Skip button 已套用 dark neon productivity 視覺。
