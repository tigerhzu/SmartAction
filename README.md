...
```text
SmartAction-Release-v1.3.0.zip
```
...
- 10 種高品質黑曜石金屬輪盤主題，包含幽靈、鯊魚、櫻花、霓虹與宇宙動態軌道。
- Classic、Cute 與 Woven Light 三套全域 UI；Cute 內建高畫質預設背景，Woven Light 光點背景會隨滑鼠產生視差與局部擾動。
- Windows 系統匣與應用程式 Logo 會跟隨目前的全域 UI 風格切換。
- 12 種星座背景，可自訂星座連線與星星顏色；中央星空在所有輪盤主題中都會保留。
...
1. 下載並解壓縮 `SmartAction-Release-v1.3.0.zip`。
...
- 修改全域快捷鍵。
- 選擇 Tiger、Purple、Ice、Lava、Cosmic、Halloween、Kawaii、Sakura、Cyber 或 Ocean 輪盤主題。
- 選擇 12 星座背景並調整星座顏色。
- 可開啟「Reduce ring motion」停用輪盤、Hover、點擊與開場動畫。
- 新增、刪除、排序及編輯輪盤動作。
- 匯出或匯入完整設定檔。
- 設定是否隨 Windows 啟動。
