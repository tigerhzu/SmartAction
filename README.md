...
目前公開發行版本為 `v1.3.0`；主線程式碼正整合以 Core service 與 authenticated loopback API 為基礎的 Web Control Center，取代舊式大型原生管理視窗。此版本已加入 Core／API／Web cutover 的回歸測試；發行前仍應完成完整測試與 Windows release checklist。
